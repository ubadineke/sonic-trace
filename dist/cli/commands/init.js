...
async function init(options) {
    try {
        ...
        if (fs.existsSync(outputPath)) {
            console.error(`Error: File already exists at ${outputPath}. Use a different path or remove the existing file.`);
            process.exit(1);
        }
        ...
        console.error(`Error generating config.yaml: ${error.message}`);
        process.exit(1);
    }
}
