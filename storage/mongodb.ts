...
class PostgreSQLStorage implements Storage {
  ...
  async initialize(): Promise<void> {
    ...
    `);
    await this.pool.query(
      `CREATE INDEX IF NOT EXISTS idx_wallet_address ON ${this.table} (wallet_address);`
    );
    await this.pool.query(
      `CREATE INDEX IF NOT EXISTS idx_signature ON ${this.table} (signature);`
    );
  }
  ...
  async query(filter: any): Promise<any[]> {
    const result = await this.pool.query(
      `SELECT * FROM ${this.table} WHERE wallet_address = $1`,
